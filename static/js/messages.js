...
// Obtenemos el contenedor de los mensajes
let message_body = $('.msg_card_body');
// Obtenemos el formulario de envío de mensajes
let send_message_form = $('#send-message-form')

// Obtenemos el ID del usuario logueado
const USER_ID = $('#logged-in-user').val();

// Obtenemos la pagina del usuario
let loc = window.location;

// Definimos el protocolo de inicio de la conexión
let wsStart = 'ws://'

// Si la pagina es https, cambiamos el protocolo a wss
if(loc.protocol === 'https:') {
    wsStart = 'wss://'
}

// Obtenemos el endpoint de la conexión
// Un endpoint es un punto final de comunicación - la URL completa 
// a la que nos conectaremos para establecer la conexión WebSocket.
// Se compone del protocolo (ws/wss), el host (dominio/IP:puerto) 
// y la ruta específica del servicio
let endpoint = wsStart + loc.host + loc.pathname

// Creamos una conexión WebSocket
...
socket.onopen = async function(e)
{
    ...
    // Creamos un objeto con el mensaje y el ID del usuario que lo envió
    let data = {
        'message': message,
        'sent_by': USER_ID,
        'send_to': send_to,
...
function newMessage(message, sent_by_id, thread_id) {
    ...
    // Agregamos el mensaje al contenedor y hacemos scroll hacia abajo
...
$('.contact-li').on('click', function() {
    ...
    // Hide all message wrappers
    $('.messages-wrapper').removeClass('is_active').hide();
    
    // Show only the selected chat wrapper
    $('.messages-wrapper[chat-id="' + chat_id +'"]').addClass('is_active').show();
})
